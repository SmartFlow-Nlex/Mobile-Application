...
/**
 * The web stand-in for the map panel, used only when no Mapbox token is set.
 *
 * With a token the browser shows the real Mapbox map, the same one the phone
 * shows - see SegmentMap.web.tsx. Without one there is nothing to draw tiles
 * with, since react-native-maps has no web build. Rather than an empty box,
 * this plots the same road geometry the native map draws - the real centreline
 * for this stretch, both carriageways, coloured by the same live statuses - as
 * a schematic.
 *
...
const SchematicSegmentMap: React.FC<SegmentMapProps> = ({
  segment,
  nbColor,
  sbColor,
  exitName,
...
export default SchematicSegmentMap;
